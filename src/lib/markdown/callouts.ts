/**
 * Obsidian's callouts (`> [!note] Title`), which GitHub reads as alerts. Each
 * type has an alias family, a color and an icon; an unknown type is a note.
 */

/** Path data from Phosphor's regular weight (MIT), on its 256 grid. */
const ICONS = {
  pencil:
...
type Kind = { icon: IconName; color: string };

const BLUE = "light-dark(oklch(55% 0.17 258), oklch(72% 0.13 252))";
const CYAN = "light-dark(oklch(58% 0.11 205), oklch(76% 0.1 205))";
const GREEN = "light-dark(oklch(56% 0.14 150), oklch(74% 0.14 150))";
const YELLOW = "light-dark(oklch(64% 0.14 80), oklch(80% 0.13 85))";
...
function define(names: string[], kind: Kind) {
  for (const name of names) KINDS[name] = kind;
}
define(["note"], { icon: "pencil", color: BLUE });
define(["abstract", "summary", "tldr"], { icon: "clipboard", color: CYAN });
define(["info"], { icon: "info", color: BLUE });
define(["todo"], { icon: "checkCircle", color: BLUE });
define(["tip", "hint", "important"], { icon: "fire", color: CYAN });
define(["success", "check", "done"], { icon: "check", color: GREEN });
define(["question", "help", "faq"], { icon: "question", color: YELLOW });
define(["warning", "caution", "attention"], { icon: "warning", color: ORANGE });
define(["failure", "fail", "missing"], { icon: "x", color: RED });
define(["danger", "error"], { icon: "lightning", color: RED });
define(["bug"], { icon: "bug", color: RED });
define(["example"], { icon: "list", color: PURPLE });
define(["quote", "cite"], { icon: "quote", color: GRAY });

export function calloutKind(type: string): Kind {
  return KINDS[type.toLowerCase()] ?? KINDS.note!;
}

export type CalloutHead = {
  type: string;
  /** `-` folds by default, `+` opens by default; either makes it foldable. */
  fold: "-" | "+" | null;
  /** The explicit title, or null for the type's name. */
  title: string | null;
  /** Offset of the marker's `[`, and of the title text, in the line. */
  markerFrom: number;
  titleFrom: number;
};
...
/** Reads the first line of a blockquote as a callout header, if it is one. */
export function parseCalloutHead(line: string): CalloutHead | null {
  const m = HEAD.exec(line);
  if (!m) return null;
  const [, lead, type, fold, space, title] = m as unknown as [string, string, string, string, string, string];
  const markerFrom = lead.length;
  const titleFrom = markerFrom + type.length + 3 + fold.length + space.length;
  return { type, fold: (fold || null) as CalloutHead["fold"], title: title.trim() || null, markerFrom, titleFrom };
}
